...
View 値からの内部 view 生成と sampled／storage texture の Binding、compute pass への group 設定を実装した。copy footprint／texture copy、raster と Browser 実装は未実装である。Texture の実機試験は生成・binding・診断・所有の確認であり、今回の buffer compute／readback の成功を Texture の転送・sampling・描画の成功とは扱わない。詳細は [進捗記録](../designs/graphics-implementation-progress.md) に記載する。
