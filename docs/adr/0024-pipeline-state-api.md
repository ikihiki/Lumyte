...
Browser 接続、shader package／loader との統合は未実装である。同一 description の別 handle 間での native pipeline 共有は行っていない。これは公開契約で要求する機能ではなく、必要に応じて加える内部の最適化とする。実機検証の範囲と結果は [進捗記録](../designs/graphics-implementation-progress.md) に記載する。
