...
- `ModelEnvironment` による equirectangular 環境画像、回転・強度と OcclusionTexture／OcclusionStrength。内部 GPU pass が拡散照明 32×16、鏡面反射 128×64×8 粗さ段階の atlas、BRDF LUT 128×128 を RGBA16Float で生成する。拡散・鏡面は 128 sample、BRDF は 512 sample の基準実装。GGX／相関 Smith の single-scatter split-sum 近似を使い、鏡面の粗さ段階を補間する。環境ごとの二画像と共通 LUT の内容世代を計 17 entry まで保持し、回転・強度だけの変更で再生成しない。Occlusion は環境照明だけに掛け、直接光と Emissive には掛けない。背景画像の描画は追加しない。
...
環境照明の準備と内容世代 cache は、各 Passes project の `Models/NativeModelEnvironment.cs`／`Models/PortableModelEnvironment.cs` に置く。前処理 shader はそれぞれ `Models/Shaders/Environment.slang`／`Environment.wgsl`、公開の環境入力型は共通 Passes の `Models/Data/ModelMaterialData.cs` に置く。

未実装の範囲:

- texture／color morph、Khronos asset による完全適合。従って **glTF core と五拡張の完了条件はまだ満たしていない**。loader は Lumyte.Resources の責務である。IBL の高周波 HDR 環境の品質評価、解像度・sample 数の選択、multiple scattering の補償は未実装である。
