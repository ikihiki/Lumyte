...
| `GpuBackendOptions` | 要求 feature と limit、diagnostics の有効化を指定する生成設定。shader binding の device 全体 profile は持たない。 |
...
`DirectRootData` は Portable backend の初期化条件とする。直接入力が未対応の runtime は初期化を失敗させる。対応する shader の byte 数は device の `MaxImmediateSize` に収める。
...
以下は repository root からの目標配置である。`Lumyte.Graphics.Portable` とそのテスト project は新設予定とし、既存の WebGPU project をこの契約へ改編する。
...
| `src/graphics/Lumyte.Graphics.Portable.Tests/Device/` | 新設予定の xUnit project。設定と公開値の CPU 上の振る舞いを検証する。 |
...
Portable の低層を独立させ、共通 RenderGraph の Portable provider から利用する。resource と内部 memory の所有を一つにし、明示した binding を用いる。独立 package、backend interface、直接入力の初期化条件と各 backend の動作確認は未実装である。
