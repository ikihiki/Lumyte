...
| `src/graphics/Lumyte.Graphics.Portable.Tests/Commands/` | 新設予定の xUnit project。root/動的引数のコピー、未提出記録の破棄など Lumyte が所有する記録契約を検証する。 |
...
Portable 固有の pass、明示 binding、直接入力を採用する。記録形式、group 設定、提出時 encode と不要な native barrier 抽象の整理は未実装である。
