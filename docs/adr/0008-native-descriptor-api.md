...
両 backend の compute shader から混在 heap と sampler を参照する実機検証を追加した。Vulkan は Slang の unified descriptor stride と storage の一致を確認した。vertex／mesh raster にも同じ storage を接続し、heap ごとの所有と caller による選択を維持する。製品用 shader toolchain の移行は未実装である。
