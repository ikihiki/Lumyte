...
両 backend の compute shader から混在 heap と sampler を参照する実機検証を追加した。Vulkan は Slang の unified descriptor stride と storage の一致を確認した。vertex／mesh raster にも同じ storage を接続し、heap ごとの所有と caller による選択を維持する。Native offline compiler、MSBuild、package／loader を機能 pass に接続した。Output／2D／標準画像処理の shader を製品 project の build で生成する。
