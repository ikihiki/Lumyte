...
受渡し後の同期障害を通知する `GpuSubmissionException` と、その signal point・内部記録を保持する経路を実装した。上位 Resources 管理層は自身の token と遅延回収を接続し、利用終了を確認できない場合は保持して drain を失敗させる。これらは arena／pool utility の責務には含めない。下位の例外は上位 Resources の token 型を参照しない。
