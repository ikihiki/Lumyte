...
| R4 非同期診断 | [0026](../adr/0026-command-submission-and-synchronization.md)／[0027](../adr/0027-webgpu-backend-implementation.md) で GPU 使用終了と成功を区別し、0029〜0033 の upload／export／cache へ接続 | Resources の token・回収・package 成功公開へ接続済み。Graph の export／cache 接続は後続。[実装進捗](graphics-implementation-progress.md) を参照 |
...
2026-09-14 に [ADR 0028](../adr/0028-resource-utilities.md) の utility を arena／pool の明示的な貸出・返却へ限定し、依存関係が必要な寿命、提出 token、遅延回収と非同期転送を [ADR 0029](../adr/0029-resource-management-api.md) の上位管理層へ集約した。二つの Resources assembly は維持する。utility が貸出の返却可否を推測せず、上位が全利用と必要な依存を終了した後に返却する。utility と管理層を実装済みとし、RenderGraph への接続は後続に残す。
