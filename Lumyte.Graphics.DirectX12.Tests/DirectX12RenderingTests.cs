...
public sealed class DirectX12RenderingTests
{
    ...
    private const string BufferPixelSource = """
        ByteAddressBuffer shaderBuffers[64] : register(t0, space2);

        float4 bufferPixel() : SV_Target0
        {
            return asfloat(shaderBuffers[0].Load4(0));
        }
        """;
    ...
    public void BufferIdCanBeReadByAPixelShader()
    {
        ...
        GpuShaderPackage package = CreatePackage(
            abiHash, VertexSource, "triangleVertex", BufferPixelSource, "bufferPixel");
        GpuRasterPipelineHandle pipeline = device.CreateRasterPipeline(
            new GpuRasterPipelineDescription([new(GpuFormat.Rgba8Unorm)]),
            package,
            "triangleVertex",
