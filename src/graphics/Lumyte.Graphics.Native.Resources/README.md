...
`Lumyte.Graphics.Native.Resources` は Native backend の heap をまとめて確保し、整列した範囲の貸出・返却・再利用を行うライブラリです。[ADR 0028](../../../docs/adr/0028-resource-utilities.md) の Native memory arena を実装しています。参照先は `Lumyte.Graphics.Native` だけで、backend、Portable、shader library、資産 loader、RenderGraph へ依存しません。
