...
namespace Lumyte.Graphics.WebGPU.Tests.Synchronization;

public sealed class WebGpuTimelineTests
{
    [Fact]
    public async Task InitialValueHasNoGpuWorkOrDiagnosticsToWaitFor()
    {
        const ulong initial = (1ul << 40) + 7;
        using var timeline = new WebGpuTimeline(new object(), new(), initial);
    ...
    {
        using var timeline = new WebGpuTimeline(new object(), new(), 10);
        timeline.Reserve(11);
    ...
    {
        using var timeline = new WebGpuTimeline(new object(), new(), 0);
        timeline.Reserve(7);
    ...
    public async Task DeviceLossEndsPendingWaitsWithoutClaimingGpuCompletion()
    {
        ...
        using var timeline = new WebGpuTimeline(new object(), status, 0);
    ...
    {
        using var timeline = new WebGpuTimeline(new object(), new(), ulong.MaxValue - 3);
    ...
    public void ForeignQueueCannotUseTheTimeline()
    {
        ...
        using var timeline = new WebGpuTimeline(owner, new(), 0);
    ...
    {
        var timeline = new WebGpuTimeline(new object(), new(), 0);
    ...
    private static void Accept(WebGpuTimeline timeline, ulong value, Task gpuEnded, Task<IReadOnlyList<P.GpuDiagnostic>> diagnostics)
    {
