# Lumyte.Graphics.Library

`Lumyte.Graphics.Library` contains reusable draw and effect contributions built on the render graph in
...
## Common pass extensions

- `AddClear` clears a color attachment and declares a write at `ColorOutput`.
- `AddFullscreen` records a three-vertex procedural full-screen triangle using a caller-supplied material.
- `AddBlit` is a full-screen pass whose material declares exactly one sampled texture.
- `AddComposite` is a full-screen pass whose material declares at least two sampled textures.
- `AddCompute` binds declared shader-data buffers, records a dispatch, and declares each buffer as read, write,
  or read/write at `ComputeShader`. Execution requires a backend that exposes compute commands.

Every raster extension has overloads for an imported target and for a target already declared by another graph
